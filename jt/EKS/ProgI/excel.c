/*
* Excel VL Beispiel: 2D Arrays als Array von Zeigern
*/
#include <stdio.h>
#include <stdlib.h>
...
int main(void) {
	...
	// Speicher allozieren
	if (NULL == (cells = malloc(rows * sizeof(int*))))
		exit(EXIT_FAILURE);
	for (row = 0; row < rows; row++)
		if (NULL == (cells[row] = malloc(cols * sizeof(int))))
			exit(EXIT_FAILURE);
